...
public class StressMeter : MonoBehaviour
{
  ...
  //  public AudioSource audioSource;
    ...
    private void Update()
    {
        ...
        if (transform.position.x <= lightBounds.position.x && !deathSigh.activeInHierarchy && !deathFade.activeInHierarchy)
        {
            AddStress(0.3f);
        }
        else if (transform.position.x >= lightBounds.position.x)
        {
            SubStress(0.1f);
        }
